...
public interface HitCountsRepository extends CrudRepository<HitCount, Long> {
    ...
    @Query(nativeQuery = true, value="SELECT hc.post_count FROM hit_counts hc WHERE hc.post_id LIKE ?1")
    int hitCountByPost(Long id);

    @Query(nativeQuery = true, value="SELECT hc.profile_count FROM hit_counts hc WHERE hc.user_id LIKE ?1")
